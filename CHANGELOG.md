...
* Serve the 2026-07-28 lifecycle (SEP-2575): `Builder::buildStateless()` produces `Mcp\Server\Stateless\StatelessProtocol`, a dispatcher for the revision that dropped the `initialize` handshake — every request carries its own `_meta` (protocol version, client capabilities, client info) and `server/discover` returns what `initialize` used to. `StatelessHttpTransport` serves it over one POST-only endpoint, with no session to keep. A per-era `Mcp\Server\Wire\WireCodecInterface` decides what a result carries on the wire, so `resultType` and the caching hints are stamped only for the revisions that define them.
